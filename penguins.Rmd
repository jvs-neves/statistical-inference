...
- A validação cruzada K-fold foi empregada como ferramenta de comparação de desempenho preditivo entre subconjuntos de variáveis candidatos, e não como critério único e automático de seleção do modelo final;
...
A análise foi estruturada em cinco etapas sequenciais, detalhadas nas Seções 4 a 7 e resumidas a seguir.

### 3.1 Exploração dos dados

Exame das distribuições univariadas e bivariadas das variáveis morfológicas e categóricas, tratamento dos dados ausentes (variáveis sex e year) e avaliação da correlação entre os preditores contínuos e a resposta (Seção 4).

### 3.2 Combinação de variáveis avaliadas por validação cruzada K-fold
...
### 3.3 Análise dos resultados da validação cruzada
...
### 3.5 Diagnóstico dos pressupostos dos resíduos, intervalos de confiança e de predição
...
```{r correlacoes, fig.cap="Mapa de calor entre variáveis", echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", out.width="75%", fig.width=8, fig.height=4.5}
library(ggcorrplot)
...
Com $F = 37{,}91$ e $p < 0,001$, rejeita-se a hipótese nula de que os dois modelos possuem a mesma capacidade explicativa: a espécie contribui de forma estatisticamente significativa para a predição do comprimento da nadadeira. Essa constância motivou sua inclusão em todos os subconjuntos candidatos avaliados a seguir.
